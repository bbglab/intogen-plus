...
wget -c http://hgdownload.soe.ucsc.edu/goldenPath/hg38/liftOver/hg38ToHg19.over.chain.gz -O ${PREPROCESS}/hg38ToHg19.over.chain.gz

echo -e "\tDownloading pileup mappability files"
bgdata get --force intogen/coverage/${INTOGEN_GENOME}
# Redo the download (it will not download the data again) to force to get the .master file
bgdata get --force intogen/coverage/${INTOGEN_GENOME}
