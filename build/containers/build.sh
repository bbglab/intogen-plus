...
if [ "${SKIP_BUILD}" == "no" ]; then
	...
	echo -e "${GREEN}${BOLD}All images built successfully for tag ${TAG}: ${YELLOW}${CONTAINERS_FMT}${CYAN}${RESET}"
fi
...
if [ "${PUSH}" == "yes" ]; then
	...
	echo -e "${GREEN}${BOLD}All images pushed successfully for tag ${TAG}: ${YELLOW}${CONTAINERS_FMT}${CYAN}${RESET}"
fi
