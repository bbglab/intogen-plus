...
config_file = '/home/fmuinos/projects/intogen-plus/methods/mutrate/mutrate.conf'
...
def compute_bp_expected(expected_dict, syn_sites):
    ...
    for sample in expected_dict:
        bp_expected[sample] = {}
        for c in expected_dict[sample]:
            bp_expected[sample][c] = expected_dict[sample][c] / syn_sites[c]
    return bp_expected
...
@click.option('--output_path', '-o', 'output_path', required=True,
              help='Path to results .pickle.gz dictionary: {gene : {sample : {context : expectation}}}',
              type=click.Path())
def compute_mutrate(annotmuts_path, genemuts_path, mut_catalogue_path, sigfit_exposures_path, cores, output_path):
    ...
    result = {}
    gene_set = ['TP53']  # gene_set = genemuts['gene_name'].unique()
    ...
    with Pool(cores) as pool:
        for a in tqdm(pool.imap(run_task, gene_set)):
            result.update(a)
