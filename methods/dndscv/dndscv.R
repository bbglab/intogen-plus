...
dndscv = function(mutations, gene_list = NULL, refdb = "hg19", sm = "192r_3w", kc = "cgc81", cv = "hg19", max_muts_per_gene_per_sample = 3, max_coding_muts_per_sample = 3000, use_indel_sites = T, min_indels = 5, maxcovs = 20, constrain_wnon_wspl = T, outp = 3) {
	message("Modificada por Santi, version 2")
    ## 1. Environment
    ...
    if (outp > 2) {
		...
		#model = suppressWarnings(glm.nb(n_indused ~ offset(log(exp_unif)) + . , data = nbrdf)) ## Añadir nueva condicion,  ¿tryctach? para correr el if siguiente si hay error, no solo si hay warning
		model = tryCatch({
			glm.nb(n_indused ~ offset(log(exp_unif)) + . , data = nbrdf)
			}, warning = function(w){
				message("Santi: warning generado corro la version sin covariates")
				glm.nb(n_indused ~ offset(log(exp_unif)) - 1 , data = nbrdf)
			}, error = function(e){
				message("Santi: error generado corro la version sin covariates")
				glm.nb(n_indused ~ offset(log(exp_unif)) - 1 , data = nbrdf)
			})
		print(paste("Santi: evaluo la condición que substituye el model que da error", !is.null(model$th.warn), nrow(genemuts)<500))
		if (!is.null(model$th.warn) | nrow(genemuts)<500) { # If there are warnings or if <500 genes, we run the regression without covariates
                    model = glm.nb(n_indused ~ offset(log(exp_unif)) - 1 , data = nbrdf)
                    message(sprintf("    Regression model for indels: no covariates were used (theta = %0.3g)", model$theta))
                } else {
