...
if __name__ == "__main__":
    ...
    muts.to_csv(os.path.join(tmp_path, "mutations_snv.csv"), sep='\t', index=False)

    # RUN DECONSTRUCTSIGN IN R
    command = 'cd ' + tmp_path + ' && Rscript ' + os.path.join(os.path.abspath(os.path.dirname(__file__)), "deconstructSigs.r")+ ' ' + os.path.join(tmp_path, "mutations_snv.csv")
    print(command)
    ...
    if not os.path.exists(os.path.join(tmp_path, "signatures_weight.csv")):
    ...
    # READ EXPOSURE FILE
    H = pd.read_csv(os.path.join(tmp_path, "signatures_weight.csv"), header=0, sep="\t")
    ...
    results_new.to_csv(out_path, sep="\t", header=True, index=False, compression="gzip")
